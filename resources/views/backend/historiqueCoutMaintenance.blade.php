...
@section('content')
   ...
   <div class="container">
      <h2 style="text-align:center;">Coût total Intervention par mois (année {{ $annee }})</h2>
    ...
    <form method="GET" action="{{ url('panel/historique_cout_maintenance') }}" style="text-align:center; margin-bottom: 20px;">
        <div class="row">
    ...
    <div class="col-md-2 my-3 flex-end" style="position: absolute; top:140px; right:90px;">
          <button class="btn btn-primary pull-right" onclick="exportPDF()"> Exporter en PDF</button>
    </div>
    <div class="col-md-2 my-3 flex-end" style="position: absolute; top:140px; right:-80px;">
          <a href="{{ url('panel/intervention_tech') }}" class="btn btn-secondary pull-right">Retour</a>
    </div>
   ...
   <div class="card">
       <div class="container">
        <h3>Coût Intervention technique (par mois)</h3>
...
<script>
   ...
   async function exportPDF() {
    ...
    logoImg.onload = function () {
       ...
       pdf.text("Rapport Mensuel - Coût Intervention technique ({{ $annee }} @if($vehiculeNom) - {{ $vehiculeNom }} @endif)", pdfWidth / 2, 20, { align: 'center' });
        ...
        // Enregistre le PDF
        pdf.save("rapport_Intervention_Technique_{{ $annee }}.pdf");
    };
}
