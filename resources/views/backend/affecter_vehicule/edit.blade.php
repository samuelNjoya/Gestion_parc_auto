...
@section('content')
 ...
 <div class="add-form-container fade-in">
        <div class="card mb-4">
            <div class="card-body">
                 <h2 class="fw-bold">Modifier l'affectation a un vehicule</h2>
                ...
                <div class="form-group side-by-side">
                            ...
                            <select id="" class="form-select" name="conducteur_id" required>
                                <option value=""  selected>Choisir un conducteur</option>
                                  @foreach ($getConducteur as $item)
                                        <option {{($getRecords->conducteur_id == $item->id) ? 'selected' : ''}} value="{{$item->id}}">{{$item->nom}}-{{$item->prenom}}</option>
                                 @endforeach
                            </select>
                 ...
                 <div class="form-group side-by-side">
                     <div class="form-group">
                        ...
                        <input type="date" name="date_debut" value="{{old('date_debut',$getRecords->date_debut)}}"  class="form-control" required />
                        <div class="invalid-feedback">Veuillez saisir une date.</div>
                    </div>
                   <div class="form-group">
                        <label for="dateIntervention" class="form-label">Date de Fin</label>
                        <input type="date" name="date_fin" value="{{old('date_fin',$getRecords->date_fin)}}"  class="form-control" required />
                        <div class="invalid-feedback">Veuillez saisir une date.</div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" class="form-control" name="description" value="" rows="2" placeholder="Description de l'intervention">{{old('description',$getRecords->description)}}</textarea>
                </div>
